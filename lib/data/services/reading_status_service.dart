...
class ReadingStatusService {
  ...
  ReadingStatusService([FirebaseFirestore? db]) : _db = db ?? FirebaseFirestore.instance;

  // values example: plan, reading, completed, dropped, on_hold
  static const fieldStatus = 'status';

  Stream<String?> watchStatus({required String userId, required String titleId}) {
    return _db
        ...
        .limit(1)
        .snapshots()
        .map((s) => s.docs.isEmpty ? null : (s.docs.first.data()[fieldStatus] as String?));
  }

    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: NestedScrollView(
                    ...
                    Positioned(
                      ...
                      bottom: 16,
                        child: StreamBuilder<bool>(
                          stream: favService.isFavoriteStream(userId: uid, titleId: item.id),
                        ...
                        child: IconButton(
                          ...
                          color: isFav ? Colors.pinkAccent : Colors.white,
                                ...
                                onPressed: () => favService.toggleFavorite(userId: uid, titleId: item.id),
                          ),
                            );
                          },
                      ),
          ...
          body: TabBarView(
            children: [
              _AboutTab(item: item, uid: uid, statusService: statusService),
...
class _AboutTab extends StatelessWidget {
  ...
  const _AboutTab({required this.item, required this.uid, required this.statusService});
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          if (uid != null) _ReadingStatusPanel(uid: uid!, titleId: item.id, statusService: statusService),
...
class _CommentsTabState extends State<_CommentsTab> {
        ...
        Expanded(
          child: StreamBuilder<List<CommentModel>>(
            ...
            builder: (context, snapshot) {
              ...
              if (items.isEmpty) {
                return const Center(
                  child: Text('No comments yet', style: TextStyle(color: AppColors.whiteSecondary)),
                );
              }
  ...
  Future<void> _post(String? uid) async {
    ...
    await _svc.addComment(titleId: widget.titleId, userId: uid, text: text);
    _c.clear();
  }
}
...
class _CommentTile extends StatelessWidget {
  ...
  const _CommentTile({required this.model, required this.uid, required this.svc});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(model.text, style: const TextStyle(color: AppColors.white)),
        subtitle: Text('${model.likeCount} likes', style: const TextStyle(color: AppColors.whiteSecondary)),
        trailing: uid == null
            ? null
            : StreamBuilder<bool>(
...
class _ReadingStatusPanel extends StatelessWidget {
  ...
  final String titleId;
  final ReadingStatusService statusService;
  const _ReadingStatusPanel({required this.uid, required this.titleId, required this.statusService});
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                        child: _statusBtn('plan', 'Plan to Read', const Color(0xFF2563EB), current),
              ),
              const SizedBox(width: 8),
                      Expanded(child: _statusBtn('reading', 'Reading', const Color(0xFF16A34A), current)),
              const SizedBox(width: 8),
                      Expanded(child: _statusBtn('completed', 'Completed', const Color(0xFF7C3AED), current)),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
                      Expanded(child: _statusBtn('dropped', 'Dropped', const Color(0xFFDC2626), current)),
              const SizedBox(width: 8),
                      Expanded(child: _statusBtn('on_hold', 'On Hold', const Color(0xFFF59E0B), current)),
                    ],
                  ),
            ],
              );
  ...
  Widget _statusBtn(String value, String text, Color color, String? current) {
    return SizedBox(
      height: 40,
      child: FilledButton(
        onPressed: () => statusService.setStatus(userId: uid, titleId: titleId, status: value),
        style: FilledButton.styleFrom(
          backgroundColor: current == value ? color : const Color(0xFF2A2E35),
          foregroundColor: current == value ? Colors.white : AppColors.whiteSecondary,
        ),

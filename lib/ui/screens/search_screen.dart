  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: prov.query.isEmpty
          ? _RecentAndSuggestions(recent: _recent, onTap: (q) {
              _c.text = q;
              prov.search(q);
            })
          : _SearchResults(),
    ...
    return ListView(
      ...
      children: [
        if (recent.isNotEmpty) ...[
          const Text('Recent searches', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.bold)),
        ...
        const Text('Popular tags', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          runSpacing: -6,
...
class _SearchResults extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (items.isEmpty) {
      return const Center(
        child: Text('No results', style: TextStyle(color: AppColors.whiteSecondary)),
      );
    }
      ...
      itemBuilder: (context, i) {
        ...
        return ListTile(
          leading: ClipRRect(
            ...
            child: Image.network(it.imageUrl, width: 56, height: 56, fit: BoxFit.cover),
          ),
          title: Text(it.title, style: const TextStyle(color: AppColors.white, fontWeight: FontWeight.w600)),
          subtitle: Text(
